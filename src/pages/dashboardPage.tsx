...
import { leaderboardApi, dashboardApi, courseApi, projectApi } from "../api";
...
export default function Dashboard() {
  ...
  const [visibleCount, setVisibleCount] = useState(4); // Needed for TopicBreakdownWidget
    ...
    const fetchDashboardData = async () => {
      try {
        const [leadRes, roomRes, actRes, dashRes, courseRes, projectRes] = await Promise.all([
          ...
          courseApi.getMyCourses(), // Unified Part: LMS
          projectApi.getEnrolled(user._id), // Unified Part: Projects
        ]);
        ...
        if (projectRes.data?.success) {
            setActiveProjects(projectRes.data.data.slice(0, 2));
            // Mocking tasks or fetching from a real endpoint
            setPendingTasks([
                { id: 1, title: "Implement API Auth", project: "AI Chatbot", due: "2h" },
                { id: 2, title: "Fix CSS Grid", project: "LMS UI", due: "1d" }
            ]);
        }
    ...
    <div className="mx-auto space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          ...
          <ActiveProjectsWidget projects={activeProjects} navigate={navigate} />
      ...
      <div className="space-y-3">
        {tasks.map((task: any) => (
          <div key={task.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
            <div className="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 shrink-0">
                <Clock size={16} />
            </div>
            <div className="min-w-0">
                <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{task.title}</p>
                <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">{task.project} • Due in {task.due}</p>
            </div>
          </div>
        ))}
      </div>
...
function ActiveProjectsWidget({ projects, navigate }: any) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
        <h3 className="font-black text-lg flex items-center gap-2 text-slate-900 dark:text-white mb-6">
          <FolderKanban className="text-blue-500" size={20} /> Project Sync
        </h3>
        <div className="space-y-4">
          {projects.length === 0 ? (
             <button onClick={() => navigate("/explore-projects")} className="w-full py-4 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl text-slate-400 font-bold text-sm hover:text-blue-500 hover:border-blue-500 transition-all">
                Explore Projects
             </button>
          ) : (
            projects.map((p: any) => (
                <div key={p.id || p._id} className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30">
                    <p className="font-bold text-sm text-blue-900 dark:text-blue-200 mb-2">{p.title}</p>
                    <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-blue-600">
                        <span>{p.currentParticipants || 0}/{p.maxParticipants || 4} Members</span>
